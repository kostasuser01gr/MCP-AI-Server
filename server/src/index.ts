...
try {
  const keys = getDb().prepare('SELECT provider, encrypted_key FROM api_keys WHERE is_active = 1').all() as { provider: string; encrypted_key: string }[];
  for (const k of keys) {
    try { aiRouter.addProvider(k.provider, k.encrypted_key); }
    catch { /* already registered via env var */ }
  }
} catch { /* api_keys table may not exist yet on first run */ }
